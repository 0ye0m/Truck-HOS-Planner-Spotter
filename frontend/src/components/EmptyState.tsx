...
export default function EmptyState() {
  return (
    <div className="mt-8 rounded-2xl border border-dashed border-line bg-white p-10 text-center sm:p-12">
      ...
      <p className="mx-auto mt-2 max-w-md text-sm leading-relaxed text-night-500">
      ...
      <div className="mx-auto mt-6 grid max-w-2xl grid-cols-1 gap-3 text-left sm:grid-cols-3">
        {[
          ...
          <div key={title} className="rounded-xl bg-canvas p-3.5">
            <p className="flex items-center gap-1.5 text-xs font-bold text-brand-700">
              {icon}
              {title}
            </p>
            <p className="mt-1 text-xs leading-relaxed text-night-500">{body}</p>
          </div>
        ))}
      </div>
      <p className="mt-6 inline-flex items-center gap-1.5 text-xs text-night-500">
